      ...
      <div className="knowledge-workspace">
        <section className="admin-panel knowledge-editor" id="knowledge-editor">
          ...
          <form className="knowledge-form" onSubmit={saveEntry}>
            ...
            <label>Short summary <small>Retrieval/admin text only; customers will not see this instead of the approved answer</small><textarea maxLength={500} rows={2} value={draft.summary} onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value }))} /></label>
            <label>Useful follow-up questions <small>Internal reference only; include customer-facing questions in the approved answer</small><textarea rows={3} value={draft.followUpQuestions} onChange={(event) => setDraft((current) => ({ ...current, followUpQuestions: event.target.value }))} placeholder={'What food are they eating now?\nHow old is your pet?'} /></label>
            <label>Safety notes <small>Internal retrieval guidance only; include required customer wording in the approved answer</small><textarea rows={3} value={draft.safetyNotes} onChange={(event) => setDraft((current) => ({ ...current, safetyNotes: event.target.value }))} placeholder="The All Good team can help and will recommend or refer to a vet if they consider it necessary." /></label>
            ...
            <label>Recommended product or collection links <small>Optional, one All Good Petfood product or collection URL per line</small><textarea rows={3} value={draft.recommendedProductUrls} onChange={(event) => setDraft((current) => ({ ...current, recommendedProductUrls: event.target.value }))} placeholder={'https://allgoodpetfood.co.nz/products/example-balm\nhttps://allgoodpetfood.co.nz/collections/skin-and-coat'} /></label>
            <div className="knowledge-form-row">
